...
Cả hai ở [src/lib/logger.ts](../../src/lib/logger.ts), tự gắn context (requestId/userId). `logError` còn ghi vào bảng `error_logs` (sink) cho trang `/admin`; `logWarn` chỉ ra stdout.

### Sự kiện bình thường — `logInfo`

| Loại                                                          | Helper                              | Level    |
| ------------------------------------------------------------ | ----------------------------------- | -------- |
| Sự kiện đáng ghi nhận, **không phải lỗi** (mốc nghiệp vụ...) | `logInfo(action, message, extra?)`  | **INFO** |

Cũng ở [src/lib/logger.ts](../../src/lib/logger.ts), tự gắn context. **Chỉ ra stdout** — KHÔNG vào DB, KHÔNG hiện `/admin`. Prod `LOG_LEVEL` mặc định `info` nên info **sẽ** xuất hiện → dùng **tiết kiệm** cho mốc thật sự đáng quan tâm, tránh nhiễu. `extra` vẫn phải sanitize.
