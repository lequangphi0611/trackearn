...
export const createTransaction = withActionContext(
  ...
  async (
    ...
    const type: TransactionType = lineConfig.expenseOnly
      ? "expense"
      : data.type;
    ...
    try {
      ...
      logError("createTransaction", err, {
        input: { line: data.line, type: data.type },
      });
      return {
        success: false,
        code: ErrorCode.INTERNAL_ERROR,
        error: "Không thể tạo giao dịch, thử lại.",
      };
    }
  },
);

export const updateTransaction = withActionContext(
  "updateTransaction",
  async (
    ...
    try {
      const result = await db.transaction(async (tx) => {
        ...
        if (current.sourceKind !== "manual") {
          return {
            ...
            error: "Giao dịch tự sinh không sửa trực tiếp.",
          };
        }
        ...
        if (debt) {
          ...
          await tx
            ...
            .set({
              ...
              paid: newPaid,
              counterpartyName:
                data.counterpartyName?.trim() || debt.counterpartyName,
              dueDate: data.dueDate?.trim() || null,
              settledAt,
            })
      ...
      if (!result.ok) {
        logWarn("updateTransaction", result.error, {
          code: result.code,
          input: { id: data.id },
        });
      ...
      logError("updateTransaction", err, { input: { id: data.id } });
      return {
        success: false,
        code: ErrorCode.INTERNAL_ERROR,
...
export const deleteTransaction = withActionContext(
  "deleteTransaction",
  async (
    _prev: ActionResult | null,
    formData: FormData,
    ...
    try {
      const result = await db.transaction(async (tx) => {
        ...
        if (current.sourceKind !== "manual") {
          return {
            ok: false as const,
            code: ErrorCode.CONFLICT,
            error: "Giao dịch tự sinh không xoá ở đây.",
          };
        }
        const [debt] = await tx
      ...
      if (!result.ok) {
        logWarn("deleteTransaction", result.error, {
          code: result.code,
      ...
      logError("deleteTransaction", err, { input: { id: parsed.data.id } });
      return {
        success: false,
        code: ErrorCode.INTERNAL_ERROR,
        error: "Không thể xoá, thử lại.",
      };
    }
  },
);
