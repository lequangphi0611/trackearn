...
### Issue #12 — hợp nhất entry point Xe múc & Thiết bị điện tử (menu "+" / Giao dịch dropdown → hub) + round 2 (đồng bộ QuickEntryDialog, dọn CTA trùng, ARIA)
- spec: verify-env/tests/issue-12-entry-points.spec.ts (**đã promote khỏi .tmp — giữ lại lâu dài**; đọc-chỉ-điều-hướng, không ghi DB — an toàn chạy lại nhiều lần)
- màn hình: BottomNav, DashboardNav, QuickEntryDialog (`/`), Transactions list (`/transactions/xe-muc`), Devices (`/devices`), New transaction (`/transactions/[line]/new?mode=`) — xem system-map "BottomNav" + "DashboardNav (điều hướng desktop) + hub CTA"
- src: BottomNav.tsx, DashboardNav.tsx, HubCard.tsx, QuickEntryDialog.tsx, transaction-lines.ts (getQuickEntryHref/getLineHubHref + field hubHref/quickEntryUsesHub), transactions/[line]/page.tsx, transactions/[line]/new/page.tsx, devices/page.tsx, DeviceTransactionForm.tsx, SegmentedToggle.tsx
- verified: 2026-07-05 (round 1: 13/15; round 2 (thêm 4 test mới + fix 2 assertion cũ do đổi ARIA): 21/23 — 2 fail còn lại luôn đúng 1 pattern "Thiết bị điện tử" menuitem timeout, đã re-run isolate nhiều lần thấy pass phần lớn → flakiness base-ui Menu đã biết, KHÔNG phải regression, xem ERROR.md; uncommitted tại lúc verify)
- kịch bản round 1: (1) FAB "+" (mobile, trang khác `/`) chỉ còn 1 mục "Xe múc" (không còn "Job sửa xe múc" rời), click → `/transactions/xe-muc` hiện đúng 2 HubCard "Tạo job sửa máy"/"Nhập thu / chi", mỗi CTA điều hướng đúng route; (2) FAB "Thiết bị điện tử" → `/devices`, hiện đủ 3 HubCard, "Bán máy từ kho" → `?mode=sell`, "Thu / Chi khác" → `?mode=income`; (3) DashboardNav dropdown "Giao dịch" → "Thiết bị điện tử" đi `/devices`, "Xe múc" vẫn đi hub; (4) route cũ vẫn sống.
- kịch bản round 2 (BA/Dev/UX 3-role discussion tìm ra): (5) nút header "Nhập"/"Nhập máy" KHÔNG còn trên `/transactions/xe-muc` và `/devices` (đã có HubCard trùng đích), NHƯNG vẫn còn trên `/transactions/phu-kien` (không có hub); (6) `/transactions/xe-muc/new` hiện HubCard "Tạo job sửa máy" làm cảnh báo chéo, `/transactions/phu-kien/new` thì KHÔNG (mảng không có job concept); (7) `QuickEntryDialog` mở từ FAB tại `/`, chọn "Xe múc" trong Select → hiện HubCard "Tạo job sửa máy" phía trên form (trước đây KHÔNG có, đây là gap nghiêm trọng nhất tìm ra qua thảo luận: entry point traffic cao nhất lại thiếu lối phân loại); (8) `SegmentedToggle` có `role="radiogroup"`/`role="radio"`/`aria-checked` đúng — verify bằng `getByRole("radio",{name}).toHaveAttribute("aria-checked",...)`, KHÔNG dùng check class `bg-primary` (đã lỗi thời từ khi thêm ARIA vì role đổi từ "button" → "radio").
- ⚠️ locator "Giao dịch" phải `exact:true`; locator "Nhập giao dịch" trên trang chủ mobile phải `.first()` (có cả FAB + nút inline cùng tên) — xem ERROR.md.
