...
- **quyết định có chủ đích (code-review fix, 2026-07-05)**: tab "Giao dịch" match `pathname.startsWith("/transactions")` KHÔNG match `/devices`, nên vào `/devices` (từ menu điều hướng "Thiết bị điện tử") tab "Kho" sáng thay vì "Giao dịch". Đây KHÔNG phải bug — `/devices` vừa là trang duyệt kho vừa là hub tạo giao dịch thiết bị, nội dung trang thực chất LÀ trang Kho + 3 HubCard. Đã thêm comment giải thích tại match function của tab "Kho" trong cả `BottomNav.tsx` và `DashboardNav.tsx`, KHÔNG đổi logic match.
...
- **code-review fix (2026-07-05) — roving tabindex**: `SegmentedToggle` nay đúng chuẩn WAI-ARIA APG radiogroup — chỉ option đang chọn có `tabIndex=0` (còn lại `-1`), `onKeyDown` trên wrapper xử lý `ArrowLeft`/`ArrowUp` (option trước) và `ArrowRight`/`ArrowDown` (option sau), gọi `onChange` + focus programmatically vào button mới qua `ref` array. Trước đó mỗi option là 1 Tab-stop riêng, không có xử lý phím mũi tên.
...
- src: src/app/(dashboard)/components/DashboardNav.tsx, src/lib/transaction-lines.ts (getLineHubHref/getQuickEntryHref + field hubHref/quickEntryUsesHub/repairJobHref), src/app/(dashboard)/components/HubCard.tsx, src/app/(dashboard)/components/RepairJobHubCard.tsx, src/app/(dashboard)/transactions/[line]/page.tsx, src/app/(dashboard)/devices/page.tsx, src/app/(dashboard)/transactions/[line]/new/page.tsx, src/app/(dashboard)/components/dashboard/QuickEntryDialog.tsx, src/app/(dashboard)/transactions/components/DeviceTransactionForm.tsx, src/components/forms/SegmentedToggle.tsx
- verified: 2026-07-05 (issue #12 round 1+2 + code-review fixes: repairJobHref config-driven, RepairJobHubCard tách dùng chung, SegmentedToggle roving-tabindex+arrow-key; uncommitted tại lúc verify)
